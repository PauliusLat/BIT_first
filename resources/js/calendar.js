...
class Calendar {
  ...
  render(lastDayM, curentDay, dataDate) {
    ...
    const exisitClassMonth = document.querySelector(".cview__month-current")
      .textContent;
    ...
    if (check.length == 0 && check1.length == 0) {
      ...
      Array.from(document.querySelectorAll(".cview--spacer")).forEach((el) =>
        el.remove()
      );
      Array.from(document.querySelectorAll(".cview--date")).forEach((el) =>
        el.remove()
      );
  ...
  event(action, month, day) {
    ...
    send.addEventListener("click", (e) => {
      ...
      if (sendE.length != 0) {
        axios
          .post(this.uri + this.path + "calendar-store-admin", {
            date: action,
            event: sendE,
            time: time,
          })
  ...
  renderEvents(action) {
    axios
      .post(this.uri + this.path + "calendar-create-admin", {})
  ...
  deleteEvent(id, action) {
    axios
      .post(this.uri + this.path + "calendar-delete-admin", {
        eventID: id,
      })
      .then(function(response) {
        if (response.status == 200 && response.statusText == "OK") {
      ...
      .then(function(response) {
        if (response.status == 200 && response.statusText == "OK") {
          ...
          for (let i = 0; i < dayEvents.length; i++) {
            for (let j = 0; j < keys.length; j++) {
              if (
                data[keys[j]].event_date == dayEvents[i].dataset.date &&
